...
function extractPage(): ExtractedPage {
  if (!/^https?:$/.test(location.protocol)) throw new Error("Esta página não pode ser salva");
  ...
  if (content.length < 200) throw new Error("A página não possui texto útil suficiente para salvar");
  const canonical = document.querySelector<HTMLLinkElement>('link[rel="canonical"]')?.href;
  return { title: (article?.title || document.title).trim().slice(0, 500), url: normalizeUrl(canonical || location.href), description: document.querySelector<HTMLMetaElement>('meta[name="description"]')?.content?.trim() ?? null, author: article?.byline?.trim() ?? null, language: document.documentElement.lang || "und", content };
}

chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type !== "content.extract") return;
  try { sendResponse({ ok: true, data: extractPage() }); }
  catch (error) { sendResponse({ ok: false, error: error instanceof Error ? error.message : "Falha ao extrair página" }); }
});
